...
```{r setup}
    #TODO: fill in file path to analysis_data 
    knitr::opts_knit$set(root.dir = normalizePath("/PATH/TO/home1/logan/490_cluster/analysis_data")) 
```
...
```{r Exercise 1.9}
# write code here
mtcars$transmission <- mtcars["am"] == 0
ifelse(mtcars$transmission, "automatic", "manual")
head(mtcars)
...
```{r}
write.csv(clinic, "/home1/lmoua/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/lmoua/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
birthday_year = 18371/365
print(birthday_year)
# Patient 5 is 50 years old

year = 3767/365
age = 20211/365
last_contact = age - year
print(year)
print(age)
print(last_contact)
# Patient 1 was 45 years old during the last contact.
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic$initial_pathologic_dx_year, clinic$death_days_to)
```
...
```{r Exercise 4.2}
# create plot here
boxplot(as.numeric(lymph_nodes_examined_count) ~ gender, data = clinic,
        xlab = "Gender", ylab = "lymph_nodes")
...
```{r}
male_mask <- !is.na(clinic$gender) & clinic$gender == "MALE"
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
age_years <- as.numeric(clinic$age_at_diagnosis) / 365.25
under30_over60_mask <- !is.na(age_years)
under30_over60_mask <- !is.na(clinic$age_at_diagnosis < 35 | clinic$age_at_diagnosis > 60)
```
...
```{r}
age_category = ifelse(clinic$age_at_diagnosis <= 35 , "Young", ifelse(clinic$age_at_diagnosis >35 & clinic$age_at_diagnosis < 50, "Middle", "Old"))
cleaned_clinic$age_category <- clinic$age_category
```
...
```{r}
alive_mask <- clinic$death_days_to == "[Not Applicable]" | is.na(clinic$death_days_to)
clinic$survival_time <- ifelse(alive_mask, clinic$last_contact_days_to, clinic$death_days_to)
...
The middle age group appears to have the greatest rate of survival, on average.
# 4. What age group appears to have the worst rate of survival? 
The oldest age group appears to have the worst rate of survival up until 2200 days, where the young age group seems to do the worst.
# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
The approximate. 15-year survival rate for "Old" breast cancer patients is 0.30 or 30%.
...
The p-value = 0.2, which often isn't statistically significant. This tells us that the relationship between age and breast cancer survival is not probable given our observed data.
